...
mytreedir="/hpc/pmc_vanboxtel/projects/TallFlow/3_Output/TreeBuilding/pt2322/branches_vcfs"
...
for VCF in ${myptatodir}/*/*.vcf.gz; do 
    ...
    if [ ! -z ${SAMPLE} ]; then
        ...
        grep -f <( zgrep -P ";CLONAL_SAMPLES=1;" ${VCF} | grep -P ";ABSENT_SAMPLES=11" | cut -f 1,2 ) ${mysmurfvcf} >> ${mytreedir}/${SAMPLE}_branch.vcf
    fi
