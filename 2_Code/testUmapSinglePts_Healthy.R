...
file_dir <- "/Users/ricohagelaar/Documents/Thymus_Project/FlowCytometry/TallFlow/3_Output/PythonGating/Patients/"
setwd(file_dir)


csvfiles <- c("/Users/ricohagelaar/Documents/Thymus_Project/FlowCytometry/TallFlow/3_Output/PythonGating/Patients/E5_pt530_keymarkers.csv", 
              "/Users/ricohagelaar/Documents/Thymus_Project/FlowCytometry/TallFlow/3_Output/PythonGating/Patients/E6_pt530_keymarkers.csv")

all.df <- {}
for (f in csvfiles){
  # Get the sample names
  SampleName = gsub(".*/", "", gsub("_keymarkers.csv", "", f))
  SampleNameShort <- gsub(".*_", "", SampleName)
  print(SampleName)
...
### Run umaps without healthy samples 
all.df.noHealthy <- all.df[! all.df$Celltype %in% c("B-cell-like", "Monocyte-like", "NK-cell-like", "pDC-like", 
                "Unknown"), ]
umap.input.noHealthy <- all.df.noHealthy[,1:17] %>% select(- contains("CD45"))
umap.data.noHealthy <- umap(umap.input.noHealthy, n_neighbors = perplex, n_threads = 12, n_epochs = n_iter)
all.df.noHealthy %<>% mutate(umap1 = umap.data.noHealthy[, 1],  umap2 = umap.data.noHealthy[, 2])

p3 <- ggplot(all.df.noHealthy,  aes(x = umap1, y = umap2, color = Celltype)) +geom_point(size = 0.2, show.legend = TRUE) + theme_bw()+ scale_color_manual(values = pal) + guides(color = guide_legend(override.aes = list(size = 4), ncol = 2))
p4 <- ggplot(all.df.noHealthy,  aes(x = umap1, y = umap2, color = SampleID)) +geom_point(size = 0.2, show.legend = TRUE) + theme_bw()+  guides(color = guide_legend(override.aes = list(size = 4), ncol = 2))
...
ggsave(filename = "../../../3_Output/Umaps/Patients/testSinglePtsUmap_healthy/pt530_Umap_Celltype.pdf", p1)
ggsave(filename = "../../../3_Output/Umaps/Patients/testSinglePtsUmap_healthy/pt530_Umap_SampleID.pdf", p2)
ggsave(filename = "../../../3_Output/Umaps/Patients/testSinglePtsUmap_healthy/pt530noHealthy_Umap_Celltype.pdf", p3)
ggsave(filename = "../../../3_Output/Umaps/Patients/testSinglePtsUmap_healthy/pt530noHealthy_Umap_SampleID.pdf", p4)


### Save data
saveRDS(all.df, "../../../3_Output/Umaps/Patients/testSinglePtsUmap_healthy/pt530.rds")
saveRDS(all.df.noHealthy, "../../../3_Output/Umaps/Patients/testSinglePtsUmap_healthy/pt530noHealthy.rds")
