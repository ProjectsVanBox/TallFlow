...
SampleOrder <- c("ALLBULK", "MONOBULK", "DNCD1aPos", "DP", "SPCD8")
...
for (vcf_fname in list.files(path='.',pattern='_BulkSeqSMuRF.vcf',full.names = T)) {
  ...
  ratiodf <- data.frame(DNCD1aPos = splitdf$DNCD1aPos.VAF / splitdf$ALLBULK.VAF)
  ...
  df.molten.filter <- df.molten %>% filter(bulk %in% c("DNCD1aPos", "DP", "SPCD8"))
