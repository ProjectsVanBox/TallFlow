...
setwd("~/hpc/projects/TallFlow/3_Output/TreeBuilding/pt2283D/manual_filter/")
...
upset_matrix <- upset_matrix[which(rowSums(upset_matrix) < 14),]
upsetPlot <- upset(as.data.frame(upset_matrix),nsets=ncol(upset_matrix), order.by = "freq")
head(upset_matrix)

max(rowSums(upset_matrix))
