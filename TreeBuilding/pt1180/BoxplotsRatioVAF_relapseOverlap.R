...
setwd("~/hpc/projects/TallFlow/3_Output/TreeBuilding/pt1180/branches_vcfs/overlapRelapse/")
...
SampleOrder <- c("ALLBULK", "MSCBULK","DNCD1aNeg", "DNCD1aPos")
...
for (vcf_fname in list.files(path='.',pattern='_BulkSeqSMuRF.vcf',full.names = T)) {
 ...
 if (nrow(myvcf) == 0 ) {
  ...
  df.molten.filter <- df.molten %>% filter(bulk %in% c("DNCD1aNeg", "DNCD1aPos"))
