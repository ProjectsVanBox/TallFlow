...
setwd("~/hpc/projects/TallFlow/3_Output/TreeBuilding/pt2229_new/branches_vcfs/overlapBulkSeq2/")
dir.create("OERatio", showWarnings = TRUE, recursive = FALSE, mode = "0777")
#colors
### Colour selection
...
# group comparisons
my_comparisons <- list( c("ALLBULK", "MSCBULK"), c("ALLBULK", "DNCD1aNeg"), c("ALLBULK", "DNCD1aPos"), 
                        c("ALLBULK", "iSPCD4"), c("ALLBULK", "DP"))
...
for (vcf_fname in list.files(path='.',pattern='_BulkSeqSMuRF.vcf',full.names = T)) {
  ...
  ratiodf$ALLBULK <- splitdf$ALLBULK.VAF / splitdf$ALLBULK.VAF
