...
for DONOR in /hpc/pmc_vanboxtel/projects/TallFlow/3_Output/PTATO_Dev/pt2229/snvs/*; do
    DONOR=$(basename ${DONOR} )
    for CONTROL_CL in /hpc/pmc_vanboxtel/projects/TallFlow/3_Output/PTATO_Dev/pt2229/QC/CallableLoci/${DONOR}/*MSC*callableloci.bed; do
	CONTROL=$( echo ${CONTROL_CL} | grep -oP "[\w,-]+.callableloci.bed" | cut -f1 -d'.')
	for SAMPLE_CL in /hpc/pmc_vanboxtel/projects/TallFlow/3_Output/PTATO_Dev/pt2229/QC/CallableLoci/${DONOR}/*callableloci.bed; do
	    ...
	    for SAMPLE_VCF in /hpc/pmc_vanboxtel/projects/TallFlow/3_Output/PTATO_Dev/pt2229/snvs/${DONOR}/220919_HMFreg1764_pt2229_${SAMPLE}/*${SAMPLE}.ptato.filtered.vcf.gz; do
