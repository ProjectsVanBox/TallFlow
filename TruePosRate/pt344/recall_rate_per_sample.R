...
setwd("~/hpc/pmc_vanboxtel/projects/TallFlow/3_Output/TruePosRate/pt344/")

myBulkVcf_fname <- "~/hpc/pmc_vanboxtel/projects/TallFlow/3_Output/PTATO_Dev/pt344/intermediate/short_variants/somatic_vcfs/pt344/230612_HMFreg2010_pt344.vcf.filtered_variants_dbnsfp_CosmicCodingMuts_gonl.snps_indels.r5.liftover.hg38.sorted_pt344-DX1BM-ALLBULK.SMuRF.filtered.vcf.gz"
...
TALL1_missed <- clonal_table[clonal_table[,"TALL1"] == 0 & rowSums(clonal_table) >= 8,]
TALL2_missed <- clonal_table[clonal_table[,"TALL2"] == 0 & rowSums(clonal_table) >= 8,]
TALL3_missed <- clonal_table[clonal_table[,"TALL3"] == 0 & rowSums(clonal_table) >= 8,]

TALL4_missed <- clonal_table[clonal_table[,"TALL4"] == 0 & rowSums(clonal_table) >= 8,]
TALL5_missed <- clonal_table[clonal_table[,"TALL5"] == 0 & rowSums(clonal_table) >= 8,]
TALL6_missed <- clonal_table[clonal_table[,"TALL6"] == 0 & rowSums(clonal_table) >= 8,]

TALL7_missed <- clonal_table[clonal_table[,"TALL7"] == 0 & rowSums(clonal_table) >= 8,]
TALL8_missed <- clonal_table[clonal_table[,"TALL8"] == 0 & rowSums(clonal_table) >= 8,]
TALL9_missed <- clonal_table[clonal_table[,"TALL9"] == 0 & rowSums(clonal_table) >= 8,]
