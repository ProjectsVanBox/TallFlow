...
setwd("~/hpc/pmc_vanboxtel/projects/TallFlow/3_Output/TruePosRate/pt1909/")

myBulkVcf_fname <- "~/hpc/pmc_vanboxtel/projects/TallFlow/3_Output/BulkSeq_SMuRF/pt1909/231017_HMFreg2090_pt1909_Bulk.vcf.filtered_variants_dbnsfp_CosmicCodingMuts_gonl.snps_indels.r5.liftover.hg38.sorted_pt1909-DX1BM-ALLBULK.SMuRF.filtered.vcf"

myBulkVcf <- readVcf(myBulkVcf_fname)
myBulkVcfgr <- granges(myBulkVcf)

mySingleSampleSMuRF_fname <- "~/hpc/pmc_vanboxtel/projects/TallFlow/3_Output/PTATO_Dev/pt1909/intermediate/short_variants/SMuRF/pt1909/231016_HMFreg2090_pt1909.vcf.filtered_variants_dbnsfp_CosmicCodingMuts_gonl.snps_indels.r5.liftover.hg38.sorted.SMuRF.filtered.vcf"
...
TALL1_missed <- clonal_table[clonal_table[,"TALL1"] == 0 & rowSums(clonal_table) >= 7,]
TALL2_missed <- clonal_table[clonal_table[,"TALL2"] == 0 & rowSums(clonal_table) >= 7,]
TALL3_missed <- clonal_table[clonal_table[,"TALL3"] == 0 & rowSums(clonal_table) >= 7,]

TALL4_missed <- clonal_table[clonal_table[,"TALL4"] == 0 & rowSums(clonal_table) >= 7,]
TALL5_missed <- clonal_table[clonal_table[,"TALL5"] == 0 & rowSums(clonal_table) >= 7,]
TALL6_missed <- clonal_table[clonal_table[,"TALL6"] == 0 & rowSums(clonal_table) >= 7,]

TALL7_missed <- clonal_table[clonal_table[,"TALL7"] == 0 & rowSums(clonal_table) >= 7,]
TALL8_missed <- clonal_table[clonal_table[,"TALL8"] == 0 & rowSums(clonal_table) >= 7,]
TALL9_missed <- clonal_table[clonal_table[,"TALL9"] == 0 & rowSums(clonal_table) >= 7,]
