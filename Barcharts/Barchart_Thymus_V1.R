...
file.list <-  list.files(path = "/Users/ricohagelaar/Documents/Thymus_Project/FlowCytometry/TallFlow/3_Output/PythonGating/ThymusMore/", pattern = "*.csv$", full.names = TRUE, recursive = TRUE)
combine.list <- list()
for (f in file.list){
...
write.table(LabelCountsTable, file = "./3_Output/Barcharts/Thymus/LabelCountsTable_Thymus.txt", sep = "\t", row.names = FALSE)
...
write.table(PercentagesLabel, file = "./3_Output/Barcharts/Thymus/PercentagesLabel_Thymus.txt", sep = "\t", row.names = T, col.names = NA)
...
ggsave(filename = "./3_Output/Barcharts/Thymus/Dendrogram_Thymus.pdf", p.den)
ggsave(filename = "./3_Output/Barcharts/Thymus/OrderedBargraph_Thymus.pdf", p.bar)
